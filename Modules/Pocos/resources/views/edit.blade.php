...
<div class="space-y-6 md:space-y-8 animate-fade-in pb-12">
    ...
    <div class="relative overflow-hidden bg-slate-900 rounded-3xl border border-slate-800 shadow-2xl mb-8">
        ...
        <div class="absolute top-0 right-0 -mt-12 -mr-12 w-64 h-64 bg-amber-500/10 rounded-full blur-3xl"></div>
        <div class="absolute bottom-0 left-0 -mb-12 -ml-12 w-64 h-64 bg-blue-500/10 rounded-full blur-3xl"></div>

        <div class="relative px-8 py-10 md:px-12 md:py-16">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-8">
                <div class="space-y-4">
                    <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[10px] font-black uppercase tracking-widest">
                    ...
                    <div class="space-y-2">
                        <h1 class="text-4xl md:text-5xl font-black text-white uppercase tracking-tight">
                            Editar <span class="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-400 font-light">Poço</span>
                        </h1>
                        <nav class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-500">
                            <a href="{{ route('pocos.index') }}" class="hover:text-amber-400 transition-colors">Poços Artesianos</a>
                            <x-icon name="chevron-right" class="w-2 h-2" />
                            <span class="text-amber-400">{{ $poco->codigo ?? 'Gestão de Ativo' }}</span>
                        </nav>
                    </div>
                </div>

                <div class="flex items-center gap-4">
                    <a href="{{ route('pocos.show', $poco) }}" class="group inline-flex items-center justify-center gap-2 px-6 py-3 text-[10px] font-black text-white bg-slate-800 rounded-xl hover:bg-slate-700 transition-all uppercase tracking-widest border border-slate-700">
            ...
            <div class="premium-card overflow-hidden">
                ...
                <div class="p-8 space-y-10">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                ...
                                <select form="poco-form" id="localidade_id" name="localidade_id" required
                            ...
                            <div>
                                ...
                                <input form="poco-form" type="text" id="endereco" name="endereco" value="{{ old('endereco', $poco->endereco) }}" required
                        ...
                        <div x-data="{
                            lat: '{{ old('latitude', $poco->latitude) }}',
                            lng: '{{ old('longitude', $poco->longitude) }}',
                            convertDMS(type) {
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    ...
                                    <input form="poco-form" type="text" id="latitude" name="latitude" x-model="lat" @blur="convertDMS('lat')"
                                ...
                                <div>
                                    ...
                                    <input form="poco-form" type="text" id="longitude" name="longitude" x-model="lng" @blur="convertDMS('lng')"
                        ...
                        <div class="mt-4 rounded-3xl overflow-hidden border border-slate-100 dark:border-slate-800 shadow-inner">
                            <x-map
                                ...
                                :nome-mapa="old('nome_mapa', $poco->codigo)"
                                icon-type="poco"
                                height="400px"
                                zoom="15"
                            />
                    ...
                    <div class="space-y-6 pt-10 border-t border-gray-100 dark:border-slate-800">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                ...
                                <div class="relative">
                                    <input form="poco-form" type="number" step="0.01" id="profundidade_metros" name="profundidade_metros" value="{{ old('profundidade_metros', $poco->profundidade_metros) }}" required
                                        class="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 dark:bg-slate-900/50 dark:border-slate-700 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all dark:text-white leading-relaxed">
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input form="poco-form" type="number" step="0.01" id="vazao_litros_hora" name="vazao_litros_hora" value="{{ old('vazao_litros_hora', $poco->vazao_litros_hora) }}"
                                        class="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 dark:bg-slate-900/50 dark:border-slate-700 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all dark:text-white leading-relaxed">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                ...
                                <input form="poco-form" type="text" id="diametro" name="diametro" value="{{ old('diametro', $poco->diametro) }}"
                            ...
                            <div>
                                ...
                                <input form="poco-form" type="date" id="data_perfuracao" name="data_perfuracao" value="{{ old('data_perfuracao', $poco->data_perfuracao ? $poco->data_perfuracao->format('Y-m-d') : '') }}"
                                    class="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 dark:bg-slate-900/50 dark:border-slate-700 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all dark:text-white leading-relaxed">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6 pt-10 border-t border-gray-100 dark:border-slate-800">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                ...
                                <input form="poco-form" type="text" id="tipo_bomba" name="tipo_bomba" value="{{ old('tipo_bomba', $poco->tipo_bomba) }}"
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input form="poco-form" type="number" id="potencia_bomba" name="potencia_bomba" value="{{ old('potencia_bomba', $poco->potencia_bomba) }}"
                                        placeholder="7.5"
                                        class="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 dark:bg-slate-900/50 dark:border-slate-700 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all dark:text-white leading-relaxed">
                    ...
                    <div class="border-t border-gray-100 dark:border-slate-800 pt-10">
                        ...
                        <textarea form="poco-form" id="observacoes" name="observacoes" rows="4"
                            placeholder="Notas técnicas, histórico ou alertas específicos para este poço..."
                            class="w-full px-5 py-4 bg-slate-50 border border-slate-200 dark:bg-slate-900/50 dark:border-slate-700 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all dark:text-white leading-relaxed">{{ old('observacoes', $poco->observacoes) }}</textarea>
                    </div>

                    <div class="flex items-center justify-end gap-4 pt-6 border-t border-gray-100 dark:border-slate-800">
                        <x-pocos::button href="{{ route('pocos.show', $poco) }}" variant="secondary" class="!rounded-2xl !px-8 border-slate-200">
                            Descartar Alterações
                        </x-pocos::button>
                        <x-pocos::button type="submit" form="poco-form" variant="primary" class="!rounded-2xl !px-10 !bg-amber-600 shadow-lg shadow-amber-500/20 border-none">
            ...
            <div class="premium-card p-8 space-y-6">
                ...
                <div>
                    ...
                    <select form="poco-form" id="status" name="status" required
                        class="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 dark:bg-slate-900/50 dark:border-slate-700 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all dark:text-white leading-relaxed">
                        <option value="ativo" {{ (old('status') ?? $poco->status) == 'ativo' ? 'selected' : '' }}>100% Operacional</option>
                        <option value="inativo" {{ (old('status') ?? $poco->status) == 'inativo' ? 'selected' : '' }}>Fora de Serviço</option>
                        <option value="manutencao" {{ (old('status') ?? $poco->status) == 'manutencao' ? 'selected' : '' }}>Manutenção Preventiva</option>
                        <option value="bomba_queimada" {{ (old('status') ?? $poco->status) == 'bomba_queimada' ? 'selected' : '' }}>Falha Crítica (Bomba)</option>
                    </select>
                </div>
