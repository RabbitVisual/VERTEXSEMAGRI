...
<div class="space-y-6 md:space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 md:pb-6 border-b border-gray-200 dark:border-slate-700">
        <div>
            ...
            <nav aria-label="breadcrumb" class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                <a href="{{ route('admin.pocos.index') }}" class="hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">Poços</a>
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
                </svg>
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('pocos.show', $poco->id) }}" class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 focus:ring-4 focus:ring-emerald-300 dark:bg-emerald-600 dark:hover:bg-emerald-700 dark:focus:ring-emerald-800 transition-colors">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
    ...
    <!-- Informações do Poço - Flowbite Card -->
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Informações do Poço</h3>
        </div>
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Código</label>
                    <p class="text-sm text-gray-900 dark:text-white font-medium">{{ $poco->codigo }}</p>
                </div>
                <div>
                    ...
                    @php
                        ...
                        $statusClass = $statusCores[$poco->status_cor] ?? 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300';
                        @endphp
                    <span class="px-2.5 py-0.5 text-xs font-medium rounded-full {{ $statusClass }}">{{ $poco->status_texto }}</span>
                </div>
                @if($poco->localidade)
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Localidade</label>
                    <a href="{{ route('admin.localidades.show', $poco->localidade->id) }}" class="text-sm text-emerald-600 hover:text-emerald-700 dark:text-emerald-400">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-900 dark:text-white">{{ $poco->endereco }}</p>
                </div>
                @endif
                @if($poco->equipeResponsavel)
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Equipe Responsável</label>
                    <a href="{{ route('admin.equipes.show', $poco->equipeResponsavel->id) }}" class="text-sm text-emerald-600 hover:text-emerald-700 dark:text-emerald-400">
                ...
                @if($poco->tipo_bomba)
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Tipo de Bomba</label>
                    <p class="text-sm text-gray-900 dark:text-white">{{ ucfirst($poco->tipo_bomba) }}</p>
                </div>
                @endif
                @if($poco->profundidade_metros)
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Profundidade</label>
                    <p class="text-sm text-gray-900 dark:text-white">{{ number_format($poco->profundidade_metros, 2, ',', '.') }}m</p>
                </div>
                @endif
                @if($poco->vazao_litros_hora)
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Vazão</label>
                    <p class="text-sm text-gray-900 dark:text-white">{{ number_format($poco->vazao_litros_hora, 2, ',', '.') }} L/h</p>
                </div>
                @endif
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-900 dark:text-white">{{ $poco->ultima_manutencao->format('d/m/Y') }}</p>
                </div>
                @endif
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-900 dark:text-white {{ $poco->precisaManutencao() ? 'text-red-600 dark:text-red-400 font-semibold' : '' }}">
                ...
                <div class="md:col-span-2">
                    ...
                    <p class="text-sm text-gray-900 dark:text-white whitespace-pre-wrap">{{ $poco->observacoes }}</p>
                </div>
                @endif
            </div>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Demandas Relacionadas ao Poço ({{ $demandasRelacionadas->count() }})</h3>
        </div>
        ...
        <div class="px-6 py-4 border-t border-gray-200 dark:border-slate-700">
            <a href="{{ route('admin.demandas.index', ['tipo' => 'poco', 'poco_id' => $poco->id]) }}" class="text-sm text-emerald-600 hover:text-emerald-700 dark:text-emerald-400">
                Ver todas as demandas deste poço →
            </a>
        </div>
    </div>
    @endif
    ...
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700">
        ...
        <div class="px-6 py-4 border-t border-gray-200 dark:border-slate-700">
            <a href="{{ route('admin.ordens.index', ['tipo_servico' => 'Poço']) }}" class="text-sm text-emerald-600 hover:text-emerald-700 dark:text-emerald-400">
                Ver todas as ordens de serviço de poços →
            </a>
        </div>
                </div>
                @endif
    ...
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-gray-200 dark:border-slate-700">
        ...
        <div class="p-6">
            <div class="space-y-3 text-sm text-gray-600 dark:text-gray-400">
                <p><strong>Integração Completa:</strong> O módulo Poços está totalmente integrado com os demais módulos do sistema.</p>
                <ul class="list-disc list-inside space-y-1 ml-4">
                    <li><strong>Demandas:</strong> Ao cadastrar uma demanda de poço (ex: bomba queimada, poço sem água, problemas elétricos), ela será automaticamente relacionada ao poço e poderá gerar uma ordem de serviço.</li>
                    <li><strong>Ordens de Serviço:</strong> As ordens criadas a partir de demandas de poço aparecerão aqui, permitindo acompanhar todo o fluxo de atendimento.</li>
                    <li><strong>Materiais:</strong> Os materiais utilizados nas ordens de serviço de poço (peças de bomba, filtros, etc.) são rastreados automaticamente.</li>
                    <li><strong>Equipes:</strong> A equipe responsável pelo poço pode ser definida e gerenciada.</li>
                    <li><strong>Relatórios:</strong> Todas as informações podem ser exportadas para relatórios completos.</li>
                </ul>
                <p class="mt-3"><strong>Fluxo Recomendado:</strong> Demanda (Bomba queimada/Poço sem água/Problemas elétricos) → Ordem de Serviço → Execução → Conclusão → Relatório</p>
                <p class="mt-2"><strong>Tipos Comuns de Demanda:</strong> Bomba submersa queimada, poço sem água, problemas elétricos no painel, filtros entupidos, entre outros.</p>
            </div>
        </div>
    </div>
</div>
