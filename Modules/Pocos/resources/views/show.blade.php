                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-2">
                        <h1 class="text-4xl md:text-5xl font-black text-white uppercase tracking-tight">
                            Poço <span class="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400 font-light">{{ $poco->codigo ?? '#' . $poco->id }}</span>
                        </h1>
                        <nav class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-500">
                            ...
                            <span class="text-blue-400">Dossiê do Ativo</span>
                        </nav>
                    </div>
                </div>

                <div class="flex flex-wrap items-center gap-3">
                    <a href="{{ route('pocos.print', $poco) }}" target="_blank" class="group inline-flex items-center justify-center gap-2 px-6 py-3.5 text-[10px] font-black text-emerald-400 bg-emerald-500/5 rounded-xl hover:bg-emerald-500/10 transition-all uppercase tracking-widest border border-emerald-500/20">
                        <x-icon name="print" class="w-4 h-4" />
                        Imprimir
                    </a>
                    <a href="{{ route('pocos.edit', $poco) }}" class="group inline-flex items-center justify-center gap-2 px-6 py-3.5 text-[10px] font-black text-amber-400 bg-amber-500/5 rounded-xl hover:bg-amber-500/10 transition-all uppercase tracking-widest border border-amber-500/20">
                        <x-icon name="pencil" class="w-4 h-4" />
                        Editar
                    </a>
                    <a href="{{ route('pocos.index') }}" class="group inline-flex items-center justify-center gap-2 px-6 py-3.5 text-[10px] font-black text-white bg-slate-800 rounded-xl hover:bg-slate-700 transition-all uppercase tracking-widest border border-slate-700">
            ...
            <div x-show="activeTab === 'detalhes'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2 space-y-8">
                    <div class="premium-card overflow-hidden">
                        <div class="px-8 py-6 border-b border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-900/50 flex items-center justify-between">
                            <h2 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                                <x-icon name="location-pin-lock" class="w-5 text-blue-500" />
                        ...
                        <div class="p-8">
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mb-10">
                                <div class="space-y-1.5">
                                    ...
                                    <p class="text-2xl font-black text-slate-900 dark:text-white tracking-widest">{{ $poco->codigo ?? 'AGU-N/D' }}</p>
                                </div>
                                <div class="space-y-1.5">
                                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em]">Localidade Atendida</p>
                                    <p class="text-lg font-black text-slate-700 dark:text-slate-300 uppercase tracking-tighter">{{ $poco->localidade->nome ?? 'Área não mapeada' }}</p>
                                </div>
                                <div class="space-y-1.5">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="p-5 bg-slate-50 border border-slate-100 dark:bg-slate-900/50 dark:border-slate-800 rounded-3xl flex items-center gap-4">
                                    <div class="p-3 bg-white dark:bg-slate-800 rounded-xl shadow-sm text-blue-500">
                                        <x-icon name="map-location-dot" class="w-6 h-6" />
                                    </div>
                                    <p class="text-xs font-black text-slate-700 dark:text-slate-300 uppercase tracking-tight leading-relaxed">{{ $poco->endereco }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="premium-card overflow-hidden">
                        <div class="px-8 py-6 border-b border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-900/50 flex items-center justify-between">
                            <h2 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                                <x-icon name="satellite" class="w-5 text-indigo-500" />
                            ...
                            <div class="absolute bottom-6 left-6 right-6 z-[1000]">
                                <div class="bg-white/95 dark:bg-slate-900/95 backdrop-blur-md p-5 rounded-3xl shadow-2xl border border-white/20 flex flex-wrap gap-8 justify-center">
                                    <div class="text-center group">
                                        ...
                                        <p class="text-xs font-black text-slate-900 dark:text-white font-mono group-hover:text-blue-500 transition-colors">{{ $poco->latitude ?? 'Não definida' }}</p>
                                    </div>
                                    ...
                                    <div class="text-center group">
                                        ...
                                        <p class="text-xs font-black text-slate-900 dark:text-white font-mono group-hover:text-blue-500 transition-colors">{{ $poco->longitude ?? 'Não definida' }}</p>
                                    </div>
                                    <div class="w-px h-8 bg-slate-100 dark:bg-slate-800"></div>
                ...
                <div class="lg:col-span-1 space-y-8">
                    ...
                    <div class="premium-card p-8 bg-gradient-to-br from-white to-blue-50/30 dark:from-slate-800 dark:to-slate-900/50">
                        <h2 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2 mb-6">
                            <x-icon name="user-gear" class="w-5 text-blue-500" />
                            Responsabilidade
                        </h2>
                        ...
                        <div class="flex items-center gap-4 p-5 bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-800 mb-6">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">{{ $poco->equipeResponsavel->nome ?? 'Setor Público' }}</p>
                            </div>
                        </div>

                        <div class="space-y-4">
                             <div class="flex items-center justify-between text-[11px] font-bold uppercase tracking-tight px-1 text-slate-600 dark:text-slate-400">
                                <span class="opacity-60">Última Revisão:</span>
                                <span class="font-black text-slate-900 dark:text-white">{{ $poco->ultima_manutencao ? $poco->ultima_manutencao->format('d/m/Y') : 'N/D' }}</span>
                            </div>
                            <div class="flex items-center justify-between text-[11px] font-bold uppercase tracking-tight px-1 text-rose-500">
                                <span class="opacity-60 text-slate-600 dark:text-slate-400">Próxima Revisão:</span>
                                <span class="font-black">{{ $poco->proxima_manutencao ? $poco->proxima_manutencao->format('d/m/Y') : 'N/D' }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="premium-card p-8">
                         <h2 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2 mb-6">
                            <x-icon name="comment-dots" class="w-5 text-amber-500" />
                            Logs & Notas
                        </h2>
                        <div class="p-6 bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800 rounded-3xl min-h-[150px]">
            ...
            <div x-show="activeTab === 'tecnico'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div class="premium-card overflow-hidden">
                    <div class="px-8 py-6 border-b border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-900/50">
                        <h2 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                            <x-icon name="bore-hole" class="w-5 text-blue-500" />
                            Perfuração
                        </h2>
                    </div>
                    <div class="p-8 space-y-6">
                        <div class="flex justify-between items-center group">
                            ...
                            <span class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight group-hover:text-blue-500 transition-colors">{{ $poco->profundidade_metros }} MT</span>
                        </div>
                        <div class="flex justify-between items-center group">
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Vazão Nominal</span>
                            <span class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight group-hover:text-blue-500 transition-colors">{{ $poco->vazao_litros_hora }} L/H</span>
                        </div>
                        <div class="flex justify-between items-center group">
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Diâmetro do Tubo</span>
                            <span class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight group-hover:text-blue-500 transition-colors">{{ $poco->diametro ?? 'N/D' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="premium-card overflow-hidden">
                    <div class="px-8 py-6 border-b border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-900/50">
                        <h2 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
                            <x-icon name="bolt" class="w-5 text-amber-500" />
                            Sistema Elétrico
                        </h2>
                    </div>
                    <div class="p-8 space-y-6">
                         <div class="flex justify-between items-center group">
                            ...
                            <span class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight group-hover:text-amber-500 transition-colors">{{ $poco->tipo_bomba ?? 'Desconhecido' }}</span>
                        </div>
                        <div class="flex justify-between items-center group">
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Potência Instalada</span>
                            <span class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight group-hover:text-amber-500 transition-colors">{{ $poco->potencia_bomba ?? 'N/D' }} HP/CV</span>
                        </div>
                    </div>
                </div>

                <!-- Resumo Operacional -->
                <div class="premium-card bg-slate-900 text-white p-8 flex flex-col justify-center items-center text-center space-y-4 border-slate-800">
