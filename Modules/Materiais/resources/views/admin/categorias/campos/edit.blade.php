...
@section('title', 'Editar Campo Dinâmico - Admin')
...
<div class="space-y-6 md:space-y-8 animate-fade-in font-poppins pb-12">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div class="flex items-center gap-4">
                <div class="w-14 h-14 md:w-16 md:h-16 bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-2xl flex items-center justify-center shadow-lg shadow-emerald-200 dark:shadow-none text-white transition-transform hover:rotate-3">
                    <x-icon name="pen-to-square" style="duotone" class="w-7 h-7 md:w-8 md:h-8" />
                </div>
                <div>
    ...
    <div class="p-6 mb-8 text-sm text-red-800 rounded-2xl bg-red-50 border border-red-100 dark:bg-red-900/20 dark:text-red-400 dark:border-red-800/50 flex items-start gap-3 animate-shake">
        ...
        <div>
            <p class="font-black uppercase tracking-wider text-xs mb-2">Impedimentos Técnicos:</p>
            <ul class="list-disc list-inside space-y-1 font-bold text-xs">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input type="text" id="nome" name="nome" value="{{ old('nome', $campo->nome) }}" required
                            ...
                            placeholder="Ex: Fabricante Sugerido">
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <select id="tipo" name="tipo" required
                            ...
                            <option value="text" {{ old('tipo', $campo->tipo) == 'text' ? 'selected' : '' }}>Texto Curto (Linha Única)</option>
                            <option value="number" {{ old('tipo', $campo->tipo) == 'number' ? 'selected' : '' }}>Valor Numérico (Decimal/Inteiro)</option>
                            <option value="select" {{ old('tipo', $campo->tipo) == 'select' ? 'selected' : '' }}>Lista de Seleção (Dropdown)</option>
                            <option value="textarea" {{ old('tipo', $campo->tipo) == 'textarea' ? 'selected' : '' }}>Texto Expandido (Descritivo)</option>
                            <option value="date" {{ old('tipo', $campo->tipo) == 'date' ? 'selected' : '' }}>Calendário (Data)</option>
                            <option value="boolean" {{ old('tipo', $campo->tipo) == 'boolean' ? 'selected' : '' }}>Estado Lógico (Sim/Não)</option>
                        </select>
                    </div>
                </div>

                <div id="opcoes-container" class="md:col-span-2 space-y-2 transition-all duration-500 {{ old('tipo', $campo->tipo) == 'select' ? 'block' : 'hidden' }}">
                    ...
                    <div class="relative group">
                        ...
                        <textarea id="opcoes" name="opcoes" rows="2"
                            ...
                            placeholder="Opção A, Opção B, Opção C">{{ old('opcoes', $campo->opcoes_array ? implode(", ", $campo->opcoes_array) : '') }}</textarea>
                    </div>
                    <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest pl-1 mt-1">Este conjunto só é exibido se o formato selecionado for 'Lista de Seleção'.</p>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative group">
                        ...
                        <input type="text" id="placeholder" name="placeholder" value="{{ old('placeholder', $campo->placeholder) }}"
                            class="w-full bg-slate-50 dark:bg-slate-900 border-none rounded-2xl focus:ring-2 focus:ring-emerald-500/20 text-gray-900 dark:text-white text-sm font-bold pl-12 p-4 transition-all shadow-inner"
                            placeholder="Ex: Digite o nome da marca...">
                    </div>
                </div>
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <textarea id="descricao" name="descricao" rows="3"
                        ...
                        placeholder="Oriente o usuário sobre como este atributo deve ser reportado... ">{{ old('descricao', $campo->descricao) }}</textarea>
                </div>
                ...
                <div class="flex flex-col md:flex-row gap-8 pt-4">
                    ...
                    <div class="flex items-center gap-10 pt-8">
                        <label class="relative inline-flex items-center cursor-pointer group">
                            <input type="checkbox" name="obrigatorio" value="1" {{ old('obrigatorio', $campo->obrigatorio) ? 'checked' : '' }} class="sr-only peer">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer group">
                            <input type="checkbox" name="ativo" value="1" {{ old('ativo', $campo->ativo) ? 'checked' : '' }} class="sr-only peer">
