    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 md:pb-6 border-b border-gray-200 dark:border-slate-700">
        <div>
            <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 dark:text-white flex items-center gap-3 mb-2">
                ...
                <span>Editar Campo: {{ $campo->nome }}</span>
            </h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select id="tipo" name="tipo" required
                        ...
                        <option value="text" {{ old('tipo', $campo->tipo) == 'text' ? 'selected' : '' }}>Texto Curto</option>
                        ...
                        <option value="textarea" {{ old('tipo', $campo->tipo) == 'textarea' ? 'selected' : '' }}>Texto Longo (Área de Texto)</option>
                        <option value="date" {{ old('tipo', $campo->tipo) == 'date' ? 'selected' : '' }}>Data</option>
                        <option value="boolean" {{ old('tipo', $campo->tipo) == 'boolean' ? 'selected' : '' }}>Alternador (Sim/Não)</option>
                    </select>
                </div>
                ...
                <div id="opcoes-container" class="{{ old('tipo', $campo->tipo) == 'select' ? '' : 'hidden' }}">
                    ...
                    <textarea id="opcoes" name="opcoes" rows="1"
                        class="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-full p-3 dark:bg-slate-900 dark:border-slate-700 dark:placeholder-gray-500 dark:text-white dark:focus:ring-emerald-500 dark:focus:border-emerald-500 transition-all"
                        placeholder="Opção 1, Opção 2, Opção 3">{{ old('opcoes', $campo->opcoes_array ? implode(", ", $campo->opcoes_array) : '') }}</textarea>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="descricao" name="descricao" rows="3"
                        class="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-emerald-500 focus:border-emerald-500 block w-full p-3 dark:bg-slate-900 dark:border-slate-700 dark:placeholder-gray-500 dark:text-white dark:focus:ring-emerald-500 dark:focus:border-emerald-500 transition-all"
                        placeholder="Explique o que deve ser preenchido neste campo...">{{ old('descricao', $campo->descricao) }}</textarea>
                </div>
