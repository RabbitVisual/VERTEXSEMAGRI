...
@section('title', 'Novo Campo - Admin')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 md:pb-6 border-b border-gray-200 dark:border-slate-700">
        <div>
            <h1 class="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 dark:text-white flex items-center gap-3 mb-2">
                ...
                <x-icon name="chevron-right" class="w-4 h-4" />
                <span class="text-gray-900 dark:text-white font-medium">Novo Campo</span>
            </nav>
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select id="tipo" name="tipo" required
                        ...
                        <option value="select" {{ old('tipo') == 'select' ? 'selected' : '' }}>Seleção</option>
                        <option value="textarea" {{ old('tipo') == 'textarea' ? 'selected' : '' }}>Área de Texto</option>
                        <option value="date" {{ old('tipo') == 'date' ? 'selected' : '' }}>Data</option>
                        <option value="boolean" {{ old('tipo') == 'boolean' ? 'selected' : '' }}>Sim/Não</option>
                    </select>
                </div>
                ...
                <div id="opcoes-container" class="hidden">
                    ...
                    <textarea id="opcoes" name="opcoes" rows="5"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-emerald-500 focus:border-emerald-500 block w-full p-2.5 dark:bg-slate-700 dark:border-slate-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-emerald-500 dark:focus:border-emerald-500"
                        placeholder="Ex: LED&#10;Fluorescente&#10;Incandescente">{{ old('opcoes') }}</textarea>
                ...
                <div>
                    ...
                    <textarea id="descricao" name="descricao" rows="2"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-emerald-500 focus:border-emerald-500 block w-full p-2.5 dark:bg-slate-700 dark:border-slate-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-emerald-500 dark:focus:border-emerald-500"
                        placeholder="Ex: Potência da lâmpada em watts">{{ old('descricao') }}</textarea>
                </div>
                ...
                <div class="flex items-center gap-6">
                    <div class="flex items-center">
                        <input type="checkbox" id="obrigatorio" name="obrigatorio" value="1" {{ old('obrigatorio') ? 'checked' : '' }}
