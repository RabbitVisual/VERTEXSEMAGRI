...
<div class="space-y-6">
                ...
                <div class="flex items-center gap-6">
                    ...
                    <div>
                        <div class="flex items-center gap-3 mb-2">
                            <span class="px-3 py-1 text-[10px] font-black uppercase tracking-[0.2em] bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 rounded-full">Novo Registro</span>
                            <span class="w-1 h-1 rounded-full bg-slate-700"></span>
                            <span class="text-xs font-bold text-slate-400 uppercase tracking-widest">Recursos Humanos</span>
                        </div>
                        <h1 class="text-3xl md:text-4xl font-black text-white tracking-tight">
                            Cadastrar Nova <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-blue-400">Equipe</span>
                        </h1>
                    </div>
                </div>
    ...
    @if(!$hasFuncionarios)
        <div class="p-4 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-300">
            <div class="flex items-center gap-3">
                <x-icon name="exclamation-triangle" class="w-5 h-5 flex-shrink-0" />
                <span class="text-sm font-medium">É necessário cadastrar pelo menos um funcionário antes de criar uma equipe.</span>
                <a href="{{ route('admin.funcionarios.create') }}" class="text-sm font-bold underline hover:text-amber-900 dark:hover:text-amber-200 ml-auto">Cadastrar funcionário →</a>
            </div>
        </div>
    @endif

    <form action="{{ route('equipes.store') }}" method="POST" class="max-w-4xl">
        @csrf
        ...
        <div class="bg-white dark:bg-slate-800 rounded-3xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-hidden mb-6">
            ...
            <div class="p-8 space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pl-8">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="nome" id="nome" required value="{{ old('nome') }}" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-slate-600 bg-gray-50 dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-emerald-500 focus:border-emerald-500 transition-colors" placeholder="Ex: Equipe de Manutenção Elétrica A">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="tipo" id="tipo" required class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-slate-600 bg-gray-50 dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-emerald-500 focus:border-emerald-500 transition-colors">
                            ...
                            <option value="eletricistas" {{ old('tipo') == 'eletricistas' ? 'selected' : '' }}>Eletricistas</option>
                            <option value="encanadores" {{ old('tipo') == 'encanadores' ? 'selected' : '' }}>Encanadores</option>
                            <option value="operadores" {{ old('tipo') == 'operadores' ? 'selected' : '' }}>Operadores</option>
                            <option value="motoristas" {{ old('tipo') == 'motoristas' ? 'selected' : '' }}>Motoristas</option>
                            <option value="mista" {{ old('tipo') == 'mista' ? 'selected' : '' }}>Mista</option>
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="descricao" id="descricao" rows="3" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-slate-600 bg-gray-50 dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-emerald-500 focus:border-emerald-500 transition-colors" placeholder="Breve descrição das responsabilidades desta equipe...">{{ old('descricao') }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="p-8">
                ...
                <div class="pl-8">
                    @if($hasFuncionarios)
                        <label for="funcionarios" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Selecione os Funcionários <span class="text-red-500">*</span></label>
                        <div class="relative">
                            <select name="funcionarios[]" id="funcionarios" multiple size="8" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-slate-600 bg-gray-50 dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-emerald-500 focus:border-emerald-500 transition-colors font-mono text-sm">
                                @foreach($funcionarios as $funcionario)
                                    <option value="{{ $funcionario->id }}" {{ in_array($funcionario->id, old('funcionarios', [])) ? 'selected' : '' }} class="p-2 border-b border-gray-100 dark:border-slate-600 last:border-0 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 rounded">
                    ...
                    @else
                        <div class="p-4 bg-gray-50 dark:bg-slate-700 rounded-lg text-center text-sm text-gray-500 dark:text-gray-400">
            ...
            <div class="bg-slate-50 dark:bg-slate-900/50 px-8 py-6 border-t border-gray-200 dark:border-slate-700 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="flex items-center gap-3">
                    <div class="relative inline-flex items-center cursor-pointer group">
                        <input type="checkbox" id="ativo" name="ativo" value="1" {{ old('ativo', true) ? 'checked' : '' }} class="sr-only peer">
                ...
                <div class="flex items-center gap-3 w-full sm:w-auto">
                    ...
                    <x-equipes::button type="submit" variant="primary" size="lg" class="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 border-b-4 border-indigo-800 active:border-b-0 active:translate-y-1 transition-all" @if(!$hasFuncionarios) disabled @endif>
