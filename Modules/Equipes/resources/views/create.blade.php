    ...
    <div class="bg-gradient-to-r from-emerald-600 to-emerald-700 dark:from-emerald-800 dark:to-emerald-900 rounded-2xl shadow-xl p-6 md:p-8 text-white">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold">Cadastrar Nova Equipe</h1>
                    <p class="text-emerald-100 dark:text-emerald-200 mt-2 text-sm md:text-base">
                        Preencha as informações abaixo para criar uma nova equipe de trabalho
                    </p>
                </div>
            </div>
    ...
    @if(!$hasFuncionarios)
        <x-equipes::alert type="warning" dismissible>
            <div class="flex items-center gap-2">
                ...
                <div>
                    <span class="font-medium">É necessário cadastrar pelo menos um funcionário antes de criar uma equipe.</span>
                    <a href="{{ route('funcionarios.create') }}" class="font-medium underline hover:no-underline inline-flex items-center gap-1 ml-1">
                        Cadastrar funcionário
                        <x-equipes::icon name="arrow-right" class="w-4 h-4" />
                    </a>
                </div>
            </div>
        </x-equipes::alert>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <x-equipes::card class="rounded-xl shadow-lg">
                ...
                <form action="{{ route('equipes.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="space-y-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                        ...
                        @if($hasFuncionarios)
                            ...
                            >
                                @foreach($funcionarios as $funcionario)
                                    <option value="{{ $funcionario->id }}"
                                            {{ in_array($funcionario->id, old('funcionarios', [])) ? 'selected' : '' }}
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">
                                ...
                                Segure <kbd class="px-1 py-0.5 bg-gray-200 dark:bg-gray-700 rounded">Ctrl</kbd> (ou <kbd class="px-1 py-0.5 bg-gray-200 dark:bg-gray-700 rounded">Cmd</kbd> no Mac) para selecionar múltiplos funcionários. Apenas funcionários ativos são exibidos.
                            </p>
                        @else
                    ...
                    <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                        <x-equipes::button href="{{ route('equipes.index') }}" variant="outline">
                            Cancelar
                        </x-equipes::button>
                        <x-equipes::button type="submit" variant="primary" :disabled="!$hasFuncionarios">
        ...
        <div class="lg:col-span-1 space-y-6">
            <x-equipes::card class="rounded-xl shadow-lg">
                <x-slot name="header">
                    <div class="flex items-center gap-3">
                        <div class="p-2 bg-amber-100 dark:bg-amber-900/30 rounded-lg">
                            <x-equipes::icon name="light-bulb" class="w-5 h-5 text-amber-600 dark:text-amber-400" />
                        </div>
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                            Dicas
                        </h3>
                ...
                <div class="space-y-4">
                    <div>
                        <h4 class="text-sm font-semibold text-gray-900 dark:text-white mb-2">Fluxo de Trabalho</h4>
                        <ol class="text-xs text-gray-600 dark:text-gray-400 space-y-2 list-decimal list-inside">
                            <li>Cadastre os funcionários no módulo de Funcionários</li>
                            <li>Crie a equipe e selecione os funcionários</li>
                            <li>Atribua a equipe a Ordens de Serviço</li>
                        </ol>
                    </div>
                    <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
                        <h4 class="text-sm font-semibold text-gray-900 dark:text-white mb-2">Importante</h4>
                        <ul class="text-xs text-gray-600 dark:text-gray-400 space-y-2 list-disc list-inside">
                            <li>É obrigatório selecionar pelo menos um funcionário</li>
                            <li>O tipo define a especialidade da equipe</li>
                            <li>O líder é opcional e deve ser um usuário do sistema</li>
                        </ul>
                    </div>
                </div>
            </x-equipes::card>
