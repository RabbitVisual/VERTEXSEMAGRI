...
class CoAdminDashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        $stats = Cache::remember("co_admin_dashboard_stats_{$period}", 300, function () use ($period) {
            return [
        ...
        return view('Co-Admin.dashboard', compact('stats', 'ultimas_demandas', 'os_execucao', 'chartData', 'alertas_criticos', 'period'));
    }
    ...
    private function getAlertasCriticos()
    {
        ...
        if ($materiaisBaixo > 0) {
            $alertas[] = [
                ...
                'link' => route('co-admin.materiais.index') . '?filter=estoque_baixo',
            ];
        }
        ...
        if ($demandasUrgentes > 0) {
            $alertas[] = [
                ...
                'link' => route('co-admin.demandas.index') . '?prioridade=urgente',
            ];
        }
        ...
        if ($osAtrasadas > 0) {
            $alertas[] = [
                ...
                'link' => route('co-admin.ordens.index') . '?status=em_execucao',
            ];
        }
