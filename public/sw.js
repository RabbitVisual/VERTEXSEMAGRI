 ...
 * Cache Agressivo + Pré-carregamento Completo
 * Funciona 100% offline mesmo sem acesso prévio
 * Inclui: Chat, Relatórios, Materiais e todas as melhorias
 *
 * @version 5.0.0
 * @date 2025-12-01
 */
...
const APP_SHELL = [
    ...
    '/icons/icon.svg',
    '/js/campo-offline.js',
    // Garantir que o sidebar seja cacheado através das páginas principais
    '/campo/dashboard',
    '/campo/ordens',
    '/campo/profile'
];

// Rotas do Campo que devem ser pré-cacheadas
const CAMPO_ROUTES = [
    '/campo/dashboard',
    '/campo/ordens',
    '/campo/profile',
...
// APIs que devem ser pré-cacheadas
const API_ROUTES = [
    '/api/v1/campo/ordens',
...
self.addEventListener('install', (event) => {
    ...
    event.waitUntil(
        Promise.all([
            ...
            cacheAppShell(),
            // 2. Pré-cachear todas as rotas do Campo
            preCacheCampoRoutes(),
            // 3. Pré-cachear APIs
            preCacheAPIs(),
...
// Pré-cachear rotas do Campo
async function preCacheCampoRoutes() {
    const cache = await caches.open(CACHE_VERSION);
    logger.log('[SW v5.1] Pré-cacheando rotas do Campo...');
    ...
    await Promise.all(promises);
    logger.log('[SW v5.1] Rotas do Campo pré-cacheadas');
}
...
self.addEventListener('fetch', (event) => {
    ...
    // Rotas do Campo - Cache First (já pré-cacheadas)
...
async function serveOfflinePage(request) {
    ...
    // Tentar página offline específica do Campo
    const cache = await caches.open(SHELL_CACHE);
...
<body>
    <div class="header">
        <div class="logo">📱 SEMAGRI Campo</div>
    ...
    <div class="container">
        ...
        <div>
            ...
            <a href="/campo/ordens" class="btn" style="background: rgba(255,255,255,0.2); color: white;">📋 Ver Ordens</a>
        </div>
...
console.log('[SW v5.1] ✅ Service Worker carregado - PWA 100% Offline Habilitada');
console.log('[SW v5.1] 📦 Versão:', CACHE_VERSION);
console.log('[SW v5.1] 🔄 Verificando atualizações a cada 1 hora');
console.log('[SW v5.1] 📋 Sidebar com Chat e Solicitação de Material incluídos');
