        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div class="flex items-center gap-4">
                <div class="w-14 h-14 md:w-16 md:h-16 bg-gradient-to-br from-blue-500 to-blue-700 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-200 dark:shadow-none text-white transition-transform hover:rotate-3">
                    <x-icon name="user-plus" style="duotone" class="w-7 h-7 md:w-8 md:h-8" />
                </div>
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-1">
                        Novo <span class="text-blue-600 dark:text-blue-400">Líder</span>
                    </h1>
                    <nav aria-label="breadcrumb" class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 font-medium">
                        ...
                        <span class="text-gray-900 dark:text-white font-bold">Cadastro de Gestão</span>
                    </nav>
                </div>
            </div>
    ...
    <form action="{{ route('admin.lideres-comunidade.store') }}" method="POST" class="space-y-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-visible relative z-30">
                    <div class="px-6 py-5 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between bg-gray-50/50 dark:bg-slate-900/50">
                        <h2 class="text-sm font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wider flex items-center gap-2">
                    ...
                    <div class="p-6 md:p-8">
                        <div class="mb-4">
                            <p class="text-sm text-slate-500 font-medium">Pesquise uma pessoa para preencher os dados automaticamente.</p>
                        </div>
                        <div class="relative">
                            <label for="pessoa_search" class="block mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                                Pesquisar Pessoa (Nome, NIS ou CPF)
                            </label>
                            <div class="relative group">
                                <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                    <x-icon name="magnifying-glass" class="w-5 h-5 text-slate-400 group-focus-within:text-blue-500 transition-colors" />
                                </div>
                                <input type="text" id="pessoa_search" name="pessoa_search"
                                    placeholder="Digite nome, CPF ou NIS..."
                                    autocomplete="off"
                                    class="w-full pl-12 pr-10 py-3.5 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all dark:text-white shadow-sm placeholder:text-slate-400">
                                <div class="absolute inset-y-0 right-0 pr-4 flex items-center cursor-pointer hidden" id="clear_search" onclick="limparBusca()">
                                    <x-icon name="x-mark" class="w-5 h-5 text-slate-400 hover:text-red-500 transition-colors" />
                                </div>
                            </div>

                            <!-- Search Results Dropdown -->
                            <div id="pessoa_results" class="hidden absolute left-0 right-0 top-full mt-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl shadow-xl max-h-80 overflow-y-auto z-50 divide-y divide-gray-100 dark:divide-slate-700">
                        ...
                        <div id="pessoa_selecionada" class="{{ old('pessoa_id') ? '' : 'hidden' }} mt-6 p-5 bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800/30 rounded-xl relative animate-fadeIn">
                            <div class="flex items-start gap-4">
                                <div class="w-10 h-10 bg-white dark:bg-slate-900 rounded-lg flex items-center justify-center text-emerald-600 shadow-sm">
                                    <x-icon name="user-check" style="duotone" class="w-6 h-6" />
                                </div>
                                <div class="flex-1">
                                    <p class="text-sm font-bold text-gray-900 dark:text-white" id="pessoa_nome_selecionada">{{ old('nome') }}</p>
                                    <p class="text-xs text-emerald-600 dark:text-emerald-400 mt-0.5 font-medium" id="pessoa_info_selecionada">Pessoa vinculada do CadÚnico</p>
                                </div>
                        ...
                        @error('pessoa_id')
                            <p class="mt-2 text-xs font-semibold text-red-600 uppercase tracking-wider">{{ $message }}</p>
                        @enderror
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
                    ...
                    <div class="p-6 md:p-8 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" id="nome" name="nome" value="{{ old('nome') }}" required
                                    class="w-full px-4 py-3 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all dark:text-white placeholder:text-slate-400 @error('nome') border-red-500 @enderror"
                            ...
                            <div>
                                ...
                                <input type="text" id="cpf" name="cpf" value="{{ old('cpf') }}"
                                    class="cpf-mask w-full px-4 py-3 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all dark:text-white placeholder:text-slate-400"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label for="telefone" class="block mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">Telefone</label>
                            ...
                            <div>
                                <label for="email" class="block mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">Email</label>
                        ...
                        <div>
                            ...
                            <input type="text" id="endereco" name="endereco" value="{{ old('endereco') }}" required
                                class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-slate-700 dark:text-slate-300 placeholder:text-slate-400"
                                placeholder="Sincronizado automaticamente, mas pode ser editado se necessário">
                            <p class="mt-2 text-xs text-slate-400 font-medium">Sugerido com base na localidade selecionada.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
                    <div class="px-6 py-5 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between bg-gray-50/50 dark:bg-slate-900/50">
                        <h2 class="text-sm font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider flex items-center gap-2">
                            <x-icon name="key" style="duotone" class="w-5 h-5 text-amber-500" />
                            Acesso ao Sistema
                        </h2>
                    </div>
                    <div class="p-6 md:p-8 space-y-8">
                        <div class="flex flex-wrap gap-6">
                            <label class="flex items-center gap-3 cursor-pointer group">
                                ...
                                <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider group-hover:text-slate-900 transition-colors">Criar Novo Acesso</span>
                            </label>
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="radio" name="criar_usuario" value="0" {{ old('criar_usuario') == '0' && !old('pessoa_id') ? 'checked' : '' }}
                                    ...
                                    class="w-5 h-5 text-blue-600 bg-slate-100 border-slate-300 focus:ring-blue-500 focus:ring-offset-0 dark:bg-slate-900 dark:border-slate-700">
                                <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider group-hover:text-slate-900 transition-colors">Vincular Existente</span>
                            </label>
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="radio" name="criar_usuario" value="pessoa" {{ old('pessoa_id') ? 'checked' : '' }}
                                    onchange="toggleVinculacao('pessoa')"
                                    class="w-5 h-5 text-blue-600 bg-slate-100 border-slate-300 focus:ring-blue-500 focus:ring-offset-0 dark:bg-slate-900 dark:border-slate-700">
                                <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider group-hover:text-slate-900 transition-colors">Sem Usuário</span>
                            </label>
                        </div>

                        <!-- Formulário para criar novo usuário -->
                        <div id="new-user-form" class="space-y-6 animate-fadeIn">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label for="user_name" class="block mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">Nome de Exibição <span class="text-red-500">*</span></label>
                                    <input type="text" id="user_name" name="user_name" value="{{ old('user_name') }}"
                                        class="w-full px-4 py-3 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all dark:text-white placeholder:text-slate-400">
                                </div>

                                <div>
                                    <label for="user_email" class="block mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">E-mail de Login <span class="text-red-500">*</span></label>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label for="user_password" class="block mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">Senha de Acesso <span class="text-red-500">*</span></label>
                                    <input type="password" id="user_password" name="user_password"
                                        class="w-full px-4 py-3 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all dark:text-white">
                                </div>

                                <div>
                                    <label for="user_password_confirmation" class="block mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">Repetir Senha <span class="text-red-500">*</span></label>
                        ...
                        <div id="existing-user-form" class="hidden animate-fadeIn">
                            <label for="user_id" class="block mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">Escolher Usuário do Banco <span class="text-red-500">*</span></label>
            ...
            <div class="space-y-8">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-700 p-6 md:p-8 space-y-8">
                    <div>
                        ...
                        <label for="status" class="block mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">Situação Inicial <span class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label for="observacoes" class="block mb-2 text-xs font-semibold text-slate-500 uppercase tracking-wider">Notas e Observações</label>
                ...
                <div class="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/20 rounded-3xl p-6">
                    <div class="flex items-start gap-4">
                        ...
                        <div>
                            <h4 class="text-sm font-bold text-blue-900 dark:text-blue-300 uppercase tracking-wider mb-1">Ajuda Rápida</h4>
                            <p class="text-xs text-blue-700 dark:text-blue-400/80 leading-relaxed">O líder de comunidade é responsável por validar pagamentos e gerenciar o poço vinculado.</p>
                        </div>
                    </div>
                </div>
            </div>
...
function buscarPessoas() {
    ...
    pessoaSearchTimeout = setTimeout(() => {
        resultsDiv.innerHTML = '<div class="p-4 text-xs font-semibold uppercase text-slate-400 text-center">Buscando...</div>';
        ...
        fetch(`{{ route('admin.lideres-comunidade.pessoas.buscar') }}?search=${encodeURIComponent(search)}&localidade_id=${localidadeId}`)
            ...
            .then(pessoas => {
                if (pessoas.length === 0) {
                    resultsDiv.innerHTML = '<div class="p-4 text-xs font-semibold uppercase text-slate-400 text-center">Nenhum resultado encontrado</div>';
                } else {
                    resultsDiv.innerHTML = pessoas.map(p => `
                        <div class="p-4 hover:bg-blue-50 dark:hover:bg-slate-700/50 cursor-pointer border-b border-gray-100 dark:border-slate-700 last:border-0 group transition-colors"
                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-tight group-hover:text-blue-600 transition-colors">${p.nome}</p>
            ...
            .catch(error => {
                ...
                resultsDiv.innerHTML = '<div class="p-4 text-xs font-semibold uppercase text-red-400 text-center">Erro ao buscar dados</div>';
            });
    }, 400);
}
