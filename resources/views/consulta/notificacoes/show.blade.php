...
@section('title', 'Detalhes da Notificação - Consulta')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 md:pb-6 border-b border-gray-200 dark:border-slate-700">
        <div>
            ...
            <nav aria-label="breadcrumb" class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                ...
                <a href="{{ route('consulta.notificacoes.index') }}" class="hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Notificações</a>
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
                </svg>
        ...
        <a href="{{ route('consulta.notificacoes.index') }}" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 text-gray-700 dark:text-gray-300 rounded-xl font-medium transition-all duration-300">
                ...
                <div class="p-6 space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            @endphp
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $tipoClasses[$tipo] ?? $tipoClasses['info'] }}">
                    ...
                    <div class="pt-4 border-t border-gray-200 dark:border-slate-700">
                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">Mensagem</label>
                        <div class="p-4 bg-gray-50 dark:bg-slate-900/50 rounded-lg border border-gray-200 dark:border-slate-700">
                            <p class="text-sm text-gray-900 dark:text-white whitespace-pre-line">{{ $notification->message ?? 'Sem mensagem' }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div>
                        ...
                        <p class="text-base font-semibold text-gray-900 dark:text-white">#{{ $notification->id }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Destinatário</label>
                        <p class="text-sm text-gray-900 dark:text-white">{{ $notification->user->name ?? 'N/A' }}</p>
                    </div>
                    @endif
                    @if($notification->module_source)
                    <div>
                        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Módulo</label>
                        <p class="text-sm text-gray-900 dark:text-white">{{ $notification->module_source }}</p>
                    </div>
                    @endif
                </div>
            </div>
