...
@section('title', 'Pessoas - Consulta')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <nav aria-label="breadcrumb" class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                ...
                <span class="text-gray-900 dark:text-white">Pessoas</span>
            </nav>
        </div>
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 p-4 md:p-5">
        ...
        <h3 class="text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Com Localidade</h3>
        <p class="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">{{ $estatisticas['com_localidade'] }}</p>
    </div>
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 p-4 md:p-5">
        <div class="flex items-center justify-between mb-3">
        ...
        <h3 class="text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Sem Localidade</h3>
        <p class="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">{{ $estatisticas['sem_localidade'] }}</p>
    </div>
    <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 p-4 md:p-5">
        <div class="flex items-center justify-between mb-3">
            <div class="w-10 h-10 bg-gradient-to-br from-indigo-500 to-indigo-600 rounded-lg flex items-center justify-center">
                <svg class="w-5 h-5 text-white" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m-3-2.818l.818.182A2.25 2.25 0 0015.75 15h3a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0018.75 4.5h-3a2.25 2.25 0 00-2.182 1.682l-.818.182m0 0V4.5m0 0h-3a2.25 2.25 0 00-2.25 2.25v7.5A2.25 2.25 0 006.75 15h3M9 12.75h.008v.008H9v-.008z" />
                </svg>
            </div>
        </div>
        <h3 class="text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Recebem PBF</h3>
        <p class="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">{{ $estatisticas['recebem_pbf'] }}</p>
    </div>
</div>
@endif
...
<div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-700 overflow-hidden mb-6">
    ...
    <div class="p-4 md:p-6">
        <form method="GET" action="{{ route('consulta.pessoas.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Buscar</label>
                <input type="text" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Nome..." class="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
            </div>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Recebe PBF</label>
                <select name="recebe_pbf" class="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    <option value="">Todos</option>
                    <option value="1" {{ ($filters['recebe_pbf'] ?? '') === '1' ? 'selected' : '' }}>Sim</option>
                    <option value="0" {{ ($filters['recebe_pbf'] ?? '') === '0' ? 'selected' : '' }}>Não</option>
                </select>
            </div>
            ...
            <div class="md:col-span-2 lg:col-span-4 flex gap-2">
                ...
                <a href="{{ route('consulta.pessoas.index') }}" class="inline-flex items-center justify-center px-4 py-2 border border-gray-300 dark:border-slate-600 text-sm font-medium rounded-lg text-gray-700 dark:text-gray-300 bg-white dark:bg-slate-700 hover:bg-gray-50 dark:hover:bg-slate-600">
                    <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
...
<div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-700 overflow-hidden">
    <div class="px-4 md:px-6 py-4 md:py-5 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50">
        <h2 class="text-base md:text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
            ...
            Lista de Pessoas (Dados Protegidos - LGPD)
        </h2>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
            <thead class="bg-gray-50 dark:bg-slate-800">
                <tr>
                    ...
                    <th class="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden md:table-cell">NIS</th>
            ...
            <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                    <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-medium text-gray-900 dark:text-white">{{ LgpdHelper::maskName($pessoa->nom_pessoa) }}</span>
                        </div>
                    </td>
                    <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400 hidden md:table-cell">
                        <code class="px-2 py-1 bg-gray-100 dark:bg-slate-700 rounded text-xs">{{ LgpdHelper::maskNis($pessoa->num_nis_pessoa_atual) }}</code>
                    </td>
                    <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400 hidden lg:table-cell">
                        <code class="px-2 py-1 bg-gray-100 dark:bg-slate-700 rounded text-xs">{{ LgpdHelper::maskCpf($pessoa->num_cpf_pessoa) }}</code>
                    </td>
                    <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-600 dark:text-gray-400 hidden md:table-cell">{{ $pessoa->localidade->nome ?? '-' }}</td>
                    <td class="px-4 md:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <a href="{{ route('consulta.pessoas.show', $pessoa->id) }}" class="inline-flex items-center justify-center w-8 h-8 text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title="Visualizar">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                <tr>
                    <td colspan="5" class="px-6 py-12 text-center">
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">Nenhuma pessoa encontrada.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
