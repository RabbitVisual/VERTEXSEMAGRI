    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white flex items-center gap-3 mb-2">
                ...
                <span>{{ LgpdHelper::maskName($pessoa->nom_pessoa) }}</span>
            ...
            <nav aria-label="breadcrumb" class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                ...
                <a href="{{ route('consulta.pessoas.index') }}" class="hover:text-blue-600 dark:hover:text-blue-400">Pessoas</a>
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
                </svg>
        ...
        <a href="{{ route('consulta.pessoas.index') }}" class="inline-flex items-center gap-2 px-4 py-2 border-2 border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-300 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-700 overflow-hidden">
            <div class="px-4 md:px-6 py-4 md:py-5 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50">
                <h2 class="text-base md:text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                    ...
                    Informações da Pessoa (Dados Protegidos - LGPD)
                </h2>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <p class="text-sm text-gray-900 dark:text-white font-medium">{{ LgpdHelper::maskName($pessoa->nom_pessoa) }}</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <code class="px-2 py-1 bg-gray-100 dark:bg-slate-700 rounded text-sm text-gray-900 dark:text-white">{{ LgpdHelper::maskNis($pessoa->num_nis_pessoa_atual) }}</code>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <code class="px-2 py-1 bg-gray-100 dark:bg-slate-700 rounded text-sm text-gray-900 dark:text-white">{{ LgpdHelper::maskCpf($pessoa->num_cpf_pessoa) }}</code>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <code class="px-2 py-1 bg-gray-100 dark:bg-slate-700 rounded text-sm text-gray-900 dark:text-white">{{ LgpdHelper::maskBirthDate($pessoa->dta_nasc_pessoa ?? $pessoa->data_nascimento) }}</code>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <a href="{{ route('consulta.localidades.show', $pessoa->localidade->id) }}" class="text-sm text-blue-600 hover:text-blue-700 dark:text-blue-400">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Recebe PBF</label>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $pessoa->recebe_pbf ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-400' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300' }}">
                            {{ $pessoa->recebe_pbf ? 'Sim' : 'Não' }}
                        </span>
                    </div>
                </div>
            </div>
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-gray-200 dark:border-slate-700 overflow-hidden">
            <div class="px-4 md:px-6 py-4 md:py-5 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50">
                <h2 class="text-base md:text-lg font-bold text-gray-900 dark:text-white">Informações</h2>
            </div>
            <div class="p-4 md:p-6">
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">ID</label>
                        <p class="text-sm text-gray-900 dark:text-white">#{{ $pessoa->id }}</p>
                    </div>
                    <div>
