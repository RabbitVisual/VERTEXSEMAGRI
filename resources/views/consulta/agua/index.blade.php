    ...
    <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 p-4 md:p-6">
            <div class="flex items-center justify-between mb-3">
                <div class="w-10 h-10 md:w-12 md:h-12 bg-gradient-to-br from-cyan-500 to-cyan-600 rounded-lg flex items-center justify-center">
                    <svg class="w-5 h-5 md:w-6 md:h-6 text-white" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                </div>
            </div>
            <h3 class="text-xs md:text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Total de Redes</h3>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 p-4 md:p-6">
            ...
            <h3 class="text-xs md:text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Ativas</h3>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 p-4 md:p-6">
            ...
            <h3 class="text-xs md:text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Inativas</h3>
            <p class="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">{{ $estatisticas['inativas'] ?? 0 }}</p>
        </div>
    </div>
    @endif
                ...
                <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                        <td class="px-4 md:px-6 py-4">
                            ...
                            @if($rede->codigo)
                            <div class="text-xs text-cyan-600 dark:text-cyan-400">{{ $rede->codigo }}</div>
                            @endif
                        </td>
                        <td class="px-4 md:px-6 py-4 whitespace-nowrap hidden sm:table-cell">
                            <span class="text-sm text-gray-900 dark:text-white">{{ $rede->localidade->nome ?? 'N/A' }}</span>
                        </td>
                        <td class="px-4 md:px-6 py-4 whitespace-nowrap hidden md:table-cell">
                            <span class="text-sm text-gray-900 dark:text-white">{{ $rede->extensao ?? 'N/A' }} m</span>
                        </td>
                        <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                            ...
                            @endphp
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $statusClasses[$status] ?? $statusClasses['ativa'] }}">
